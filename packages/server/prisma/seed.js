...
function makeDemoAssets() {
    const demoQr = (label) => `https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(`企业微信·${label}（演示扫码）`)}`;
    const stores = [
    ...
    const houseTypes = ['开间', '一室一厅', '两室一厅', '三室一厅', 'Loft'];
    ...
    return { stores, apartments: apartments, houses };
}
async function main() {
    // Seed demo assets
    ...
    const reportStatuses = [null, 'PENDING', 'SUCCESS', 'FAILED'];
    const firstNames = [
    ...
    if (needToCreate > 0) {
        ...
        if (freeHouses.length < needToCreate) {
            ...
            console.log(`Only ${freeHouses.length} free houses, cannot reach ${CONTRACT_TARGET_BEFORE_CH} base contracts. Created ${existingCount + freeHouses.length} total.`);
        }
        ...
        // eslint-disable-next-line no-console
        console.log(`Demo contracts: ${existingCount} -> ${totalAfter} (added ${totalAfter - existingCount}). Target ${CONTRACT_TARGET_BEFORE_CH} before 换房演示.`);
    }
    ...
    if (existingCh < wantChangeHouseDemos) {
        ...
        for (const old of chCandidates) {
            if (existingCh + chAdded >= wantChangeHouseDemos)
                break;
            await prisma.bill.updateMany({
                ...
                await prisma.$transaction(async (tx) => {
                    ...
                    const nc = await tx.contract.create({
                        data: {
                            ...
                            latestRentDueDate: old.latestRentDueDate,
        ...
        if (chAdded > 0) {
            // eslint-disable-next-line no-console
            console.log(`Demo 换房：已生成 ${chAdded} 组「旧合同已终止 + 新合同待签字」关联，请在合同列表「来源」列查看。`);
        }
        else if (existingCh === 0) {
            // eslint-disable-next-line no-console
            console.warn('未写入换房演示：请确认已同步 62 套房源种子（含各门店空置目标房），执行 npm run db:seed 后再试。');
        }
    }
    ...
    if (existingRenew < RENEW_SEED_COUNT) {
        ...
        for (const old of renewCandidates) {
            if (existingRenew + renewAdded >= RENEW_SEED_COUNT)
                break;
    ...
    if (existingMoveOutRefunds < MOVE_OUT_SEED_COUNT) {
        ...
        for (const c of moCandidates) {
            if (existingMoveOutRefunds + moAdded >= MOVE_OUT_SEED_COUNT)
                break;
            await prisma.bill.updateMany({
                where: { contractId: c.id },
                data: { status: 'PAID', paidAt: new Date() },
    ...
    if (reminderCount < REMINDER_TARGET) {
        ...
        const extraBills = overdueBills.length >= need
            ? []
    ...
    // eslint-disable-next-line no-console
    console.log(`Seed done. 当前合同共 ${totalContracts} 条（换房演示 ${CHANGE_HOUSE_SEED_COUNT} 组；新合同在列表「关联来源」列会标明上一份旧合同号）。`);
    // eslint-disable-next-line no-console
